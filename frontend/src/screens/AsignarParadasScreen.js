...
export default function AsignarParadasScreen({ navigation, route }) {
  const rutaIdParam = route?.params?.rutaId
    ? String(route.params.rutaId)
    : '';
  ...
  // Si la pantalla recibe un rutaId desde RutaDetalleScreen,
  // cargamos automáticamente las paradas de esa ruta.
  useEffect(() => {
    if (rutaIdParam) {
      handleCambioRuta(rutaIdParam);
    }
  }, [rutaIdParam]);
  ...
  const handleCambioRuta = async (rutaId) => {
    ...
    try {
      ...
      const idsOrdenados = data.map((p) =>
        String(p.id_parada ?? p.id)
      );
  ...
  const toggleParada = (paradaId) => {
    if (!rutaSeleccionada) {
      Alert.alert(
        'Aviso',
        'Primero selecciona una ruta en la parte superior.'
      );
      return;
    }
  ...
  const guardarConfiguracion = async () => {
    ...
    if (paradasAsignadas.length === 0) {
      Alert.alert(
        'Aviso',
        'Debes asignar al menos una parada a la ruta.'
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Éxito',
        'Las paradas de esta ruta han sido actualizadas.'
      );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerParadas}>
        ...
        <Text style={styles.subLabel}>
          Toca para añadir al recorrido en orden
        </Text>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          const indexOrden = paradasAsignadas.indexOf(
            String(item.id)
          );
          ...
          return (
            <TouchableOpacity
              style={[
                styles.paradaItem,
                isSelected &&
                  styles.paradaItemSelected
              ]}
              onPress={() =>
                toggleParada(String(item.id))
              }
            >
