...
export default function RutasScreen({ navigation }) {
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      ...
      activeOpacity={0.7}
      onPress={() =>
        navigation.navigate('RutaDetalle', {
          rutaId: item.id,
        })
      }
    >
      <View
        style={[
          ...
          !item.activo && { backgroundColor: '#EEE' },
        ]}
      >
        <MaterialCommunityIcons
          name="bus"
          size={24}
          color={item.activo ? COLORS.primary : COLORS.muted}
        />
      </View>
      ...
      <View style={styles.routeInfo}>
        ...
        <Text style={styles.routeMeta}>
          {item.paradas_count ?? 0} parada(s) ·{' '}
          {item.buses_count ?? 0} bus(es)
      ...
      <View style={styles.header}>
        ...
        {isAdmin && (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('RutaForm')}
          >
            <MaterialCommunityIcons
              name="plus"
              size={20}
